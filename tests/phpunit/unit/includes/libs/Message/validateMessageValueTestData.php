...
class ValidateMessageValueTestData extends Maintenance {
	...
	public function execute() {
		...
		$tests = [
			...
			ListParamTest::class,
			MessageValueTest::class,
			ScalarParamTest::class,
		];
		foreach ( $tests as $testClass ) {
			...
			$this->validateSerialization(
				...
				array_map( static function ( $testCase ) use ( $objClass ) {
					$args = array_shift( $testCase );
					return new $objClass( ...$args );
				}, $testClass::provideConstruct() )
			);
		}
	}
...
return ValidateMessageValueTestData::class;
